      ...
      if (person) {
        ...
        bcrypt.genSalt(10, (err, salt) => {
          bcrypt.hash(newPerson.password, salt, (err, hash) => {
            ...
            newPerson
              .save()
              .then(person => res.json(person)) // to get all data back to us
...
router.post('/login', (req, res) => {
  ...
  Person.findOne({ email })
    .then(person => {
      if (!person) {
        return res.status(404).json({ emailerror: 'User not registered!' });
      }
      bcrypt
        ...
        .then(isCorrect => {
          if (isCorrect) {
            //login success
            //res.json({ msg: 'User is able to login successfully' });
            //Uses payload and creates token for user
            const payload = {
              id: person.id,
              name: person.name,
              email: person.email
